            ...
            for param in params:
                ...
                if trk_name != "":
                    ...
                    for trk in bpy.context.selected_objects:
                        ...
                        # Create handlers for this track
                        settings = context.scene.holophonix_utils.track_handler_settings
                        if settings.auto_manage_handlers:
                            self._create_handlers_for_track(context, trk, settings)

        # Trigger deferred handler management
        if context.scene.holophonix_utils.track_handler_settings.auto_manage_handlers:
            bpy.app.timers.register(
                lambda: bpy.ops.sna.manage_track_handlers(auto_triggered=True),
                first_interval=0.5
            )
            self.report({'INFO'}, 'Tracks imported and handlers created successfully!')
        else:
            self.report({'INFO'}, 'Tracks imported successfully! (Auto-management of handlers is disabled)')
        return {"FINISHED"}
